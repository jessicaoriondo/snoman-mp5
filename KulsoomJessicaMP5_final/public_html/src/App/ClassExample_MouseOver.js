...
ClassExample.prototype.resetSelection = function() {
    this.mDirectManipulator.resetTrigger();
};

// define a hit as a WC pos within 0.2 from the center position
ClassExample.prototype.detectMouseOver = function (wcX, wcY) {
    this.detectMouseOver2(wcX, wcY, true);
    
    var triggered = false;
    
    if (this.mDirectManipulator.isActivated()) {
        if (this.mDirectManipulator.isInManipulationMode()) {
            this.mDirectManipulator.manipulate(wcX, wcY);
            triggered = true;
        } else {
            triggered = this.mDirectManipulator.triggerManipulatorMode(wcX, wcY);
        }
    }
    
    if (!triggered) {
        var wcPos = [wcX, wcY];
        var parentOnce = this.mParent.selectBase(wcPos, this.mManipulator);
    
        var childOnce = false;
        for(var i = 0; i < this.mParent.numberOfChildren(); i++){
            if(this.mParent.getChildAt(i).selected(wcPos, this.mManipulator)){
                childOnce = this.mParent.getChildAt(i).selected(wcPos, this.mManipulator);
            }
        }
        
        var once = (
            parentOnce || childOnce);
            
        if (!once) {
            this.mDirectManipulator.setXformMatrix(null, null);
        }
     }
...
// define a hit as a WC pos within 0.2 from the center position
ClassExample.prototype.detectMouseOver2 = function (wcX, wcY, didLeftClick) {
    ...
    if (withInBound(parentPos, wcPos)) {
        overObj = "Parent Body";
        if (didLeftClick){
...
//            this.mDirectManipulator.setSceneNode(this.mParent);
    ...
    // 2. Left, lower arm, pulsing blue rectangle
        ...
        if (withInBound(bluePosWC, wcPos)) {
            overObj = "Lower Arm:";
            if (didLeftClick){
                this.vmShouldDrawDirectManipulator = true;
...
//                this.mDirectManipulator.setSceneNode(this.mTopChild);
        ...
        if(withInBound(yellowPosWC, wcPos)){
            overObj = "Right Arm";
            if (didLeftClick){
                this.vmShouldDrawDirectManipulator = true;
...
//                this.mDirectManipulator.setSceneNode(this.mRightChild);
        ...
        if(withInBound(purplePosWC, wcPos)){
...
//                this.mDirectManipulator.setSceneNode(this.mTopRChild);
        ...
        if(withInBound([dmPos[0], dmPos[1] + 1], wcPos)){
            overObj = "DM Rotation Knob";
        }
        else if(withInBound([dmPos[0] + 1, dmPos[1]], wcPos)){
